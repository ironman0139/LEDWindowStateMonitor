...
uses
  ...
  ExtCtrls, XMLPropStorage, ComCtrls, Serial, Synaser, mysql56conn, sqldb, INIFiles;
...
begin
  ...
  INI := TINIFile.Create('sqlhost.ini');
  host := INI.ReadString('SQL','host','');
  nameh := INI.ReadString('SQL','nameh','');
  pw := INI.ReadString('SQL','pw','');
  port := INI.ReadString('SQL','port','');


    MySQLConnection1.HostName := host;
    MySQLConnection1.UserName := nameh;
    MySQLConnection1.Password := pw;
    MySQLConnection1.Port:= StrToInt(port);
    MySQLConnection1.DatabaseName := 'LEDWindowsState';
end;
